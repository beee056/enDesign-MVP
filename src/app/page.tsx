...
const principles = [
  [Compass, "中立の立場で考える", "特定のツールや業者に偏らず、事業に合う方法を選びます。"],
  [MessageCircleMore, "難しい言葉を使わない", "専門用語は、初めての人にも分かる言葉に直します。"],
  [ShieldCheck, "無理にすすめない", "必要がなければ、作らない選択も含めてお伝えします。"],
  [HeartHandshake, "長く伴走する", "作って終わりではなく、育てていく視点で支えます。"],
] as const;
  ...
  return (
    <div className={styles.page}>
      ...
      <header className={styles.header}>
        <Link href="/" className={styles.logo} aria-label="enDesign トップページ"><span>en</span><strong>enDesign</strong></Link>
      ...
      <main>
        <section className={styles.hero}>
          <div className={styles.heroInner}>
            <div className={styles.heroCopy}>
              ...
              <ul className={styles.heroTrust}><li><MessageCircleMore aria-hidden="true" /><span>相談だけでもOK</span></li><li><HeartHandshake aria-hidden="true" /><span>制作を前提にしません</span></li><li><span className={styles.zero}>¥0</span><span>無料相談</span></li></ul>
            </div>
        ...
        <section className={styles.partnerSection}>
          ...
          <div className={styles.partnerCopy}><p className={styles.eyebrow}>A PARTNER ON THE SAME BOAT</p><h2><span>同じ船に乗るように、</span><span>一緒に考えます。</span></h2><div className={styles.principleGrid}>{principles.map(([Icon, title, text]) => <article key={title}><Icon aria-hidden="true" /><h3>{title}</h3><p>{text}</p></article>)}</div></div>
        </section>
        ...
        <section className={styles.valueSection}>
          ...
          <div className={styles.valueList}><article><Search aria-hidden="true" /><div><span>01 / RESEARCH</span><h3>顧客の言葉から強みを見つける</h3><p>口コミ、SNS、検索、競合を横断し、繰り返し現れる価値を探します。</p></div></article><article><HeartHandshake aria-hidden="true" /><div><span>02 / HUMAN TOUCH</span><h3>写真と手描きの温度を残す</h3><p>実際の写真と、人が描くイラストを使い、事業者らしい場面をつくります。</p></div></article><article><ShieldCheck aria-hidden="true" /><div><span>03 / HONESTY</span><h3>事実と推測を混ぜない</h3><p>架空の実績や口コミを作らず、本人確認が必要な情報を分けて扱います。</p></div></article></div>
        </section>
        ...
        <section id="price" className={styles.priceSection}>
          ...
          <div className={styles.priceContent}><div className={styles.priceHeading}><p className={styles.eyebrow}>PRICE & SCOPE</p><h2>料金と制作範囲</h2><p>含まれる内容と追加費用の境界を、制作前に確認します。</p></div><div className={styles.pricePanel}><div className={styles.priceNumbers}><p>LP制作 基本料金</p><strong>150,000<small>円／税別</small></strong><span>税込165,000円</span><hr /><p>事例公開にご協力いただける場合</p><strong className={styles.discount}>110,000<small>円／税別</small></strong><span>税込121,000円</span></div><div className={styles.includes}><p>基本料金に含まれるもの</p><ul>{deliverables.map((item) => <li key={item}><Check aria-hidden="true" />{item}</li>)}</ul></div></div><div className={styles.careBox}><div><span>DOMAIN / SERVER / CARE</span><h3>公開後の運用・保守</h3></div><strong>月額5,000円<small>／税別</small></strong><p>税込5,500円。ドメイン・サーバー管理、稼働確認、契約範囲内の軽微な更新に対応します。</p></div><p className={styles.priceNote}>写真は原則として事業者様にご提供いただきます。公開前修正は1回。納期は必要素材の受領後から約1か月が目安です。ページ追加や大幅な機能追加は、着手前に別途お見積りします。</p></div>
        </section>
        ...
        <section className={styles.targetSection}><p className={styles.eyebrow}>WHO WE WORK WITH</p><h2><span>実力はある。</span><span>でも、Webでは伝わりきっていない。</span></h2><p>口コミや紹介では評価されている一方、検索から来た人には違いが見えない。そんな事業者の魅力を、調査から言葉と形にします。</p><ul>{["社労士", "税理士", "行政書士", "塾・教育", "採用に悩む企業"].map((item) => <li key={item}>{item}</li>)}</ul></section>

        <section className={styles.faqSection}><div className={styles.sectionIntro}><p className={styles.eyebrow}>FAQ</p><h2>よくある質問</h2></div><div className={styles.faqList}>{faqs.map(([question, answer]) => <details key={question}><summary>{question}<span>＋</span></summary><p>{answer}</p></details>)}</div></section>
      ...
      <footer className={styles.footer}><div><Link href="/" className={styles.footerLogo}>enDesign</Link><p>いい仕事が、ちゃんと届くように。</p></div><nav aria-label="フッターナビゲーション"><Link href="/terms">利用規約</Link><Link href="/privacy">プライバシーポリシー</Link><Link href="/legal">特定商取引法表記</Link><Link href="/support">お問い合わせ</Link><Link href="/admin">管理者ログイン</Link></nav><small>© 2026 enDesign / PIVOT&amp;QUEST</small></footer>
    </div>
  );
}
