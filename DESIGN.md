...
- 指定された素材フォルダと、その配下の正式版フォルダを唯一の素材ソースとする。
- 素材一覧を「採用・不採用・要確認」に分け、配置先を決めてから実装する。
- 制作者側で簡易SVG、仮アイコン、装飾図形、代替イラストを追加しない。
- 正式版フォルダがある場合、同階層の旧版素材は使わない。
- 必要な素材がない場合は勝手に補完せず、文字・余白・色面で成立させるか依頼者へ確認する。
